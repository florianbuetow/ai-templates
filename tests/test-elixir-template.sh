#!/usr/bin/env bash
# Test script for Elixir OTP template
...
TEMPLATE_PATH="$REPO_ROOT/blueprints/elixir-otp-base"
...
main() {
    echo ""
    echo -e "${BLUE}=== Testing Elixir OTP Template ===${NC}"
    echo ""
    ...
    if ! command -v mix >/dev/null 2>&1; then
        echo -e "${RED}✗ Error: mix is not installed${NC}"
        echo "  mix should be available with your Elixir installation"
        echo ""
        exit 1
    fi
    echo -e "${GREEN}✓ mix is installed${NC}"
    echo ""
    ...
    copier copy --trust --defaults \
        ...
        --data project_description="Test OTP application" \
        --data elixir_version=1.17 \
    ...
    assert_file_exists "mix.exs" "mix.exs file"
    assert_file_exists ".formatter.exs" ".formatter.exs file"
    assert_file_exists ".credo.exs" ".credo.exs file"
    ...
    assert_file_exists "config/config.exs" "config/config.exs"
    assert_file_exists "config/codespell/ignore.txt" "config/codespell/ignore.txt"
    assert_file_exists "lib/$APP_NAME.ex" "lib/$APP_NAME.ex"
    assert_file_exists "test/test_helper.exs" "test/test_helper.exs"
    assert_file_exists "test/${APP_NAME}_test.exs" "test/${APP_NAME}_test.exs"
    ...
    # Verify _build and deps were created
    assert_dir_exists "_build" "_build directory"
    assert_dir_exists "deps" "deps directory"
    echo ""
    ...
    assert_output_contains "$OUTPUT" "Hello from $APP_NAME" "Correct output from main"
    echo ""
    ...
    # Verify _build and deps were removed
    if [ ! -d "_build" ]; then
        echo -e "${GREEN}✓ _build directory was removed${NC}"
    else
        echo -e "${RED}✗ _build directory still exists after destroy${NC}"
        exit 1
    fi

    if [ ! -d "deps" ]; then
        echo -e "${GREEN}✓ deps directory was removed${NC}"
    else
        echo -e "${RED}✗ deps directory still exists after destroy${NC}"
        exit 1
    fi
    echo ""
